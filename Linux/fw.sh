...
BCK="/tmp/fw_backup"          # Where to save the old firewall rules
# ============================================================
...
if uname -s 2>/dev/null | grep -q OpenBSD; then
    IS_BSD=true
    IS_OPENBSD=true
elif uname -s 2>/dev/null | grep -q FreeBSD; then
    IS_BSD=true
    IS_FREEBSD=true
...
if [ ! -z "$ipt" ]; then
    ...
    # FreeBSD needs pf loaded as a kernel module; OpenBSD has it built-in
